...
void getNames() {
    // Array de objetos JSON com nomes e códigos de países
...
void setScores(){
    for(size_t i = index(); i < paises.size(); i++) {
        std::cout << "\n" << paises[i].name << "\n";
        std::cout << "Wiki -> https://en.wikipedia.org/wiki/" << paises[i].name << "\n";
        std::cout << "Nota: ";
        // only using score directly cuz its faster to test
        // it should be like this -> get all individual score then do their average
        std::cin >> paises[i].score;
        writeTxt(paises[i].name, paises[i].score);
